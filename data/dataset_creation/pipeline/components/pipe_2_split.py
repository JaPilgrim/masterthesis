...
def main(folder:str,suffix=''):
    input_path = f"{folder}1_all_articles_fetched{suffix}.csv"
    output_path = f"{folder}2_articles_labeled_cleaned{suffix}.csv"
    df = pd.read_csv(input_path)
    print(f"Before dropping, articles: {len(df)}")
    drop_indices = []
    ...
    print(f"After dropping, articles: {len(df)}")
    drop_indices =[]
    text_list = df.sub_texts
    cleaned_text_list = []
    ...
    for article_index,text in enumerate(text_list):
        ...
        if len(articles_quot_truth_list) % 10000 == 0:
    ...
    print(f"Dropping {len(drop_indices)} indices due to non-matching Label & Sentence length :")
    print(drop_indices)

    # print(f"Output o {len(drop_indices)} indices due to non-matching Label & Sentence length :")
