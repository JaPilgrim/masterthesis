...
def main(folder: str, suffix=''):
    ...
    before_article_count = len(df)
    before_sentence_count = df['sentence_list'].apply(len).sum()

    print(f"Before article drop: {before_article_count} articles")
    print(f"Before article drop: {before_sentence_count} sentences")
    ...
    after_article_count = len(df)
    after_sentence_count = df['sentence_list'].apply(len).sum()

    print('Dropping ' + str(len(drop_indices)) + " articles:")
    print(drop_indices)

    print(f"After article drop: {after_article_count} articles")
    print(f"After article drop: {after_sentence_count} sentences")
