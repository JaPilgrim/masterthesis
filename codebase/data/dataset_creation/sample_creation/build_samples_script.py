...
for category in article_categories:
    df = pd.DataFrame()
    df = pd.read_csv(
        f'../../data_files/pipeline_steps/{category}_articles/6.1_sentences_filtersadded.csv')
        ...
        for text in texts:
            for to_filter in if_filter:
                ...
                df_random_sample = df_random_grouped.apply(
                    lambda x: x.sample(n=sample)).reset_index(drop=True)
                ...
                df_random.to_csv(f'../../data_files/test_samples/5th_try/{category}_{name}.csv')
